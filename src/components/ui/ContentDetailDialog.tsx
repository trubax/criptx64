...
import { updateDoc, arrayUnion, getDoc, onSnapshot, increment, serverTimestamp } from 'firebase/firestore';
  ...
  useEffect(() => {
    const fetchAndUpdateViews = async () => {
      ...
      try {
        ...
        // Imposta il conteggio iniziale
        setViewCount(postData?.stats?.views || 0);

        const now = new Date();
        const lastViewed = postData?.stats?.lastViewedAt?.[currentUser.uid]?.toDate();

        // Verifica se è necessario incrementare il contatore
        if (!lastViewed || now.getTime() - lastViewed.getTime() > 24 * 60 * 60 * 1000) {
